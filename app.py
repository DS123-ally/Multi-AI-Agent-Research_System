...
<div class="hero">
    ...
    <h1>MultiAgentX<span>Mind</span></h1>
    <p class="hero-sub">
...
# ── Run pipeline (one agent per Streamlit rerun for live progress) ─────────────
if run_btn:
    if not topic.strip():
...
if st.session_state.running and not st.session_state.done:
    ...
    topic_val = st.session_state.topic_snapshot
    results = dict(st.session_state.results)

    if step and step > 0:
        time.sleep(STEP_COOLDOWN_SEC)

    try:
        if step == 0:
            st.info("Step 1/4 — Search Agent is gathering web sources…")
                    ...
                    lambda: search_agent.invoke(
                        {
                            "messages": [
                                (
                                    "user",
                                    f"Find recent, reliable information about: {topic_val}",
                                )
                            ]
                        },
                        config=AGENT_CONFIG,
            ...
            st.info("Step 2/4 — Reader Agent is scraping the best source…")
